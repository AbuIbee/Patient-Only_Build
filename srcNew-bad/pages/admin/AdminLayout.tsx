...
function AddUserModal({ defaultRole = 'patient', onClose, onAdded }: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onAdded(); onClose();
    } catch (err: any) { toast.error(isUniqueConstraintError(err) ? uniqueIdentityMessage(err) : 'Failed: ' + err.message); }
...
function UserTable({ roleFilter, title, showAccountType = false }: {
  ...
  const saveUser = async (u: UserRow) => {
    ...
    try {
      ...
      await assertUniqueProfileIdentity({ email: normalizedEmail, phone: e.phone, ignoreProfileId: u.id });
      ...
      load();
    } catch (err: any) { toast.error(isUniqueConstraintError(err) ? uniqueIdentityMessage(err) : 'Failed: ' + err.message); resetEdit(u); }
